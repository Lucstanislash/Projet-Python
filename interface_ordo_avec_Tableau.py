...
##        ValProc=("P"+str(i+1))
        for i in range(colmax):
            if col == 0:
                Proc=Label(frame_buttons, text=ListTabl[j] ,font=("courier", 8), fg='black', bg='white',width=17)
                Proc.grid(row=lignemin+ligne, column=col,sticky='s')
            else:
                ListTabl[j]=Entry(frame_buttons,width=20)
                ListTabl[j].grid(row=lignemin+ligne, column=col)
...
ttQuantum.grid(row=3, column=6,columnspan=9,)

Quantum=Entry(justify='center',borderwidth=3,state='disabled')#case de saisie pour le quantum
Quantum.grid(row=4, column=6,columnspan=9,ipady=15,ipadx=30)

GO=Button(fenetre, text="GO", font=("calibri", 18, "bold", 'underline'), fg='white', bg='#103985', width=5, height=0,command=lambda:Tableau(NbProc))
...
affcase.grid(row=17, column=1,columnspan=3)
...
B1.grid(row=18, column=1, columnspan=2)
B2.grid(row=18, column=3, columnspan=4)
B3.grid(row=18, column=4, columnspan=7)
B5.grid(row=18, column=7, columnspan=9)
B4.grid(row=18, column=11, columnspan=12)
